...
One plugin per MCP server or tool group. A tool plugin adds helpers; it never changes, replaces or
depends on the plugin this repo builds. The contract line a workflow prints at session start is the
only link between the two: the main model reads the contracts it has and picks a helper from them.
...
- **A tool taken away inside a project**: a bare tool name in the project's `permissions` `deny`
  ...
  not get it either — the agent runs and reports the tool missing. The launcher then picks another
  helper from the contracts that are present, and the gap is named in one line.
...
plugin, and nothing outside it names one of its helpers. A plugin that is disabled or a tool that
is denied then costs a capability, never a broken flow. `tests/plugin/toolplugin.sh` proves it for
the template: it passes the contract and agent oracles against its own root and names nothing of
the base plugin.
...
workflow per job, and keep `bin/contract.sh` as it is. An enabled tool plugin widens what a launcher can do with no edit of the
base plugin: its contract lines arrive at session start like the base plugin's own.
...
0.16.0 accepted the loss and 0.18.0 leaves the decision open; the usage numbers of those two agents decide
