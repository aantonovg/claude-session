...
# Usage: [TMUX_PREFIX=corp] watch-done.sh <done-file> <session>... ; log goes to stdout.
set -u
done_file=$1; shift
IDLE_MIN=${IDLE_MIN:-8}
typeset -A idle
...
while true; do
  all=1
  for s in "$@"; do
    pane=$(tmux capture-pane -p -t "${TMUX_PREFIX:-demo}-$s" 2>/dev/null) || { echo "$(date +%T) $s: no tmux session"; idle[$s]=$IDLE_MIN; continue; }
