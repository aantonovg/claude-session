#!/bin/sh
# Prints the stop-file paths for the current Claude Code session, one per line.
...
[ -n "$claude_pid" ] && echo "$dir/session-ping-stop-$claude_pid"
[ -n "$CLAUDE_SESSION_ID" ] && echo "$dir/session-ping-stop-$CLAUDE_SESSION_ID"
exit 0
