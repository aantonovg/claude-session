...
Details moved from the description: the shim maps the target to a real codex model and reasoning effort and runs codex exec in one fixed permission set for every launch (workspace-write sandbox, approval_policy on-request, approvals_reviewer auto_review) with per-command escalation: an escalation preamble is prepended so codex requests unsandboxed execution for blocked or silently-broken actions (screenshots, simulators, GUI automation, network, out-of-workspace paths), each adjudicated by codex's risk-based auto reviewer. Pass-by-reference is mandatory in both directions: the task prompt arrives only as CODEX PROMPT FILE (the shim never reads that file) and the answer stays on disk; the shim returns only its path plus the file's last line (CODEX OUTPUT FILE chooses where). The agent, the wrapper bin/codex-exec-logged.sh and the style file bin/codex-style.md ship with the session plugin.
