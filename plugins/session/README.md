...
| Fork subagent | inherits the parent's model AND effort, cannot override either; measured: first turn reads the parent's full prefix (84.6K read, 200 written), its own later turns write into the 5m bucket; a parent `/effort` change propagates to running forks (docs) |
