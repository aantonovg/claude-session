...
/* usage:
One catalog role, one agent, one file.
...
in (paths array, default [])
ask (the job in prose, required)
out (absolute output file, or the task directory, required)
run (short key of this launch, with a task directory)
...
Use: one agent's job. Not: a flow.
*/
...
const ROLE = A.role
const OUT = A.out
// the key that tells one launch of this role from the next: a role whose layout row is a `dir` row
// writes one file per run, so the task-directory form needs it and the file form never does
...
// `out` may name the task directory instead of a file, and which of the two it is is decided by
// outForm() of the shared block — the one rule all four scripts share. A trailing slash decides
// nothing: a launcher is free to drop it, and a task directory read as a file would put the output
// of this role one level up, in `tasks/`. In the directory form the role writes the file
// lib/task-layout.md gives it (idea 8.8), so a launcher never has to know the file name of a level
// it did not write; a role with no row of that table keeps needing a path of its own, and a role
// whose row is a `dir` row needs the run key of this launch, or the second run of that role would
// write over the file of the first one (roleOutPath).
...
let FILE
try {
  if (TEXTMODE) FILE = taskDirOf(OUT)
  else {
    const form = outForm(OUT)
    FILE = form.file || roleOutPath(form.dir, ROLE, RUNKEY)
  }
...
log(`${RUN.name('role')} | role=${ROLE} class=${JOB.class} slot=${SLOT} label=${O.label} agent=${AGENT} size=${SIZE} in=${IN.length} out=${FILE}`)
...
const COUNT = BASH_AGENTS.includes(AGENT) ? 'count it with `wc -c`'
  : READ_AGENTS.includes(AGENT) ? `you have no shell: read ${FILE} back with your Read tool after writing it and count what it holds, never a number from memory`
...
const V = outVerdict(r, FILE)
if (!V.ok) return { role: ROLE, out: FILE, class: JOB.class, slot: SLOT, label: O.label, blocked: V.blocked }
return { role: ROLE, out: FILE, class: JOB.class, slot: SLOT, label: O.label, agent: AGENT, bytes: V.bytes, result: lastLine(r) }
