...
if (!OUT) return fail('args.out (one absolute path for the report file) is required')
...
async function stage(role, job, phaseName, fields, shape) {
  ...
  let r = null
  try {
    r = await agent(f.text + tail(fields.out, agentType, shape), o)
  } catch (e) {
