export const meta = {
  name: 'review',
  description: 'Review and fix loop on an existing change: code critique, evidence check of every finding by a second agent, fixes of the confirmed ones, test run; 1-3 cycles until clean. Input args { cwd, target (diff file, "a..b" range or "worktree"), class, submodes, test, out, fix (default true) }. Output: review-N.md, evidence-N.md, tests-N.md under out (default <cwd>/reviews); fixes in the working tree when fix is true. Stops with a report on any BLOCKED stage.',
...
log(`review: ${CLS} ${SUBS.join(' ') || 'no submodes'} slots ${ROW.join(' / ')}; target ${TARGET}; fix ${FIX}`)

phase('Review')
const loop = await cycle(FIX ? 3 : 1,
  async i => {
    const rev = await agent(`Code reviewer. In ${CWD} review ${targetLine}. Findings with file:line by severity (high / medium / low): logic errors, wrong or missing error handling, races, boundaries, unchecked inputs, broken contracts with unchanged code, duplicated helpers, dead code, needless complexity. Skip style. Read surrounding code only where a finding needs it. Never edit. Write ${OUT}/review-${i}.md (create ${OUT} if missing), one numbered item per finding.
Last line of your return: "VERDICT: clean" when no high or medium finding, else "VERDICT: findings <n high> <m medium>". ${STYLE}`, opts('sonnet', `review-${i}`, { agentType: 'session:code-reviewer', phase: 'Review' }))
    ...
    const ev = await agent(`Evidence checker. Read ${OUT}/review-${i}.md. For every numbered finding open the named file at the named line in ${CWD} and decide: CONFIRMED (the code does what the finding says), REFUTED (it does not; say why in one line), UNCLEAR (needs a run to tell). Do not fix anything. Write ${OUT}/evidence-${i}.md: the same numbering, the verdict, the decisive line quoted.
Last line of your return: "VERDICT: clean" when no high or medium finding is CONFIRMED or UNCLEAR, else "VERDICT: findings <n confirmed> <m unclear>". ${STYLE}`, opts('sonnet', `evidence-${i}`, { agentType: 'session:stage-researcher', phase: 'Review' }))
  ...
  (i, r) => agent(`Code fixer. In ${CWD} apply every finding marked CONFIRMED or UNCLEAR with severity high or medium in ${OUT}/evidence-${i}.md (details in ${OUT}/review-${i}.md). Smallest correct change each. ${TEST ? `Then run "${TEST}"; write ${OUT}/tests-${i}.md with PASS/FAIL and the last 20 lines on failure.` : 'No test command given: do not run tests.'} Do not commit.
...
return {
  ...
  next: FIX ? 'Read the newest review-N.md and evidence-N.md; commit from the main session.' : 'Read review-1.md and evidence-1.md.',
}
