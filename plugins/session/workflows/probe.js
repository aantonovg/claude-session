...
}, DONE_SHAPE)))).map((s, i) => ({ ...s, direction: RUNDIRS[i], n: i + 1 }))
...
const s = await stage(SYNTH, 'synthesis', 'Synthesis', {
  ...
  ask: `Question: ${ASK}\n\nThe bundles and the critique above are your whole material. Where the critique withdraws a fact, drop it from the answer and name it under the open points; where two bundles disagree, keep both claims with their pointers.${gap.length ? `\n\nThese directions never ran, so the answer is missing what they would have brought: ${CUT.join('; ')}. Name that under the open points.` : ''}`,
  out: OUT,
}, DONE_SHAPE)
