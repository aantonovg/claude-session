export const meta = {
  ...
  whenToUse: 'A question needs facts before anything is decided, and it splits into directions: one researcher per direction, a critique over the bundles, one synthesis that adds no fact of its own.',
  phases: [{ title: 'Research' }, { title: 'Critique' }, { title: 'Synthesis' }],
}
/* usage:
Facts on one question: researchers, critique, synthesis.
ask (the question, required)
directions (array, seated by the ceiling, default [ask])
in (absolute paths array, default [])
out (task directory, or a file, required)
run (short key of this launch, required)
web (boolean, public sources not the repository, default false)
depth (lite std full, default std)
class (c1-c5, default c3)
submodes (array: no-sonnet no-opus no-fable, default [])
size (small medium large, default medium)
Out: a bundle per direction, a critique under evidence/, reviews/, the answer in report.md or out, claims carry pointers; a direction cut or lost takes ok down.
Use: facts before a decision. Not: build (make), review (chain).
*/
...
// Verification-first (lib/verification.md): the output of this flow is an answer no executable
// oracle can settle, ladder level e. So it carries the two things such an object gets — a stronger
// author for the synthesis (3.5 rule 2, the uplift of the role map) and a critique of the bundles
// before the answer is written — and nothing here reviews an object a check already decides.
//
// The decisions between the stages — how many directions the ceiling of a stage seats, what a
// finished run is and what a blocked one returns — live in the shared block as pure functions, so
// a test executes them instead of reading this file.
...
const AGENTS = ['session:tools-read-write', 'session:tools-read-write-bash', 'session:tools-web']
// An agent with Bash counts its own output; one with Read but no shell reads the file back; the
// web agent has neither and can only state what it wrote.
const BASH_AGENTS = ['session:tools-read-write-bash']
const READ_AGENTS = ['session:tools-read-write']
// The stage roles, from the stamped subset of this script.
const REPO_RESEARCH = 'researcher'
const WEB_RESEARCH = 'web-researcher'
const CRITIC = 'critic'
...
const ASK = A.ask
const OUT = A.out
// the key that tells one launch from the next: the bundles and the critique of this run are files
// of a `dir` row of the layout, one file per run, and every one of them carries it
const RUNKEY = A.run == null ? '' : String(A.run).trim()
const WEB = A.web === true || A.web === 'true'
...
const DIRS = A.directions == null || (Array.isArray(A.directions) && !A.directions.length) ? [ASK] : A.directions

// Every argument error returns instead of throwing: the launcher gets the lists back and can fix
...
log(`${[CLS, ...(Array.isArray(SUBS) ? SUBS : [])].join('-')}-probe | depth=${DEPTH} directions=${Array.isArray(DIRS) ? DIRS.length : '(no array)'} web=${WEB} in=${Array.isArray(IN) ? IN.length : '(no array)'} out=${OUT || '(none)'}`)

if (!ASK) return fail('args.ask (the question in prose) is required')
if (!OUT) return fail('args.out (one absolute path for the synthesis file) is required')
...
if (!RUNKEY) return fail('args.run (a short key of this launch, e.g. r1) is required: the bundle of every direction and the critique carry it, so a second launch writes its own files instead of over the first one\'s')
if (!Array.isArray(IN)) return fail('args.in must be a JSON array of absolute paths')
if (!Array.isArray(DIRS)) return fail('args.directions must be a JSON array of direction lines')
if (!DIRS.filter(d => d && String(d).trim()).length) return fail('args.directions holds no direction: give the question or the directions it splits into')
...
// `out` is the task directory of this run, or one synthesis file inside it, decided by outForm()
// of the shared block — the same rule all four scripts read `out` by. Reading the last segment off
// the path instead would take a task directory for a file and write the bundles and the critique
// one level up, into `tasks/`. In the directory form the answer is the report of the layout.
let FORM
...
const RESULT = FORM.file || side('report')
...
// run down with it, so a question asked in five directions and answered from two says so
const SEATS = chainSeats(DEPTH, DIRS.length)
const RUNDIRS = DIRS.slice(0, SEATS.seats)
const CUT = DIRS.slice(SEATS.seats)
log(`${RUN.name('probe')} | role=${RESEARCH} directions=${RUNDIRS.length}/${DIRS.length} room=${SEATS.room} out=${OUT}`)
...
const counted = agentType => BASH_AGENTS.includes(agentType)
  ? 'count it with `wc -c`'
  : READ_AGENTS.includes(agentType)
    ? 'read it back with your Read tool after writing it and count what it holds, never a number from memory'
    : 'you have neither a shell nor a Read tool: state the number of characters you wrote'
const tail = (out, agentType, shape) => `\n\nWork only inside the directories the paths above name. Write ${out} yourself.${writeHint(out, BASH_AGENTS.includes(agentType))} Start your return with one line \`${out} <n> bytes\` giving the size of what you wrote (${counted(agentType)}), then ${shape} A file you did not write, or left empty, is not a finished job: return BLOCKED: <why> instead of a size. On a permission denial stop at once and return BLOCKED: <the denied action>.`
...
async function stage(role, job, phaseName, fields, shape) {
  ...
  // an output no oracle can settle gets a stronger author, one class step up (3.5 rule 2); the
  // researchers, whose bundles are checked by their own pointers, stay on the cheapest slot
  const JOB = bindClass(roleClass(role, CLS), SUBS)
  const o = JOB.opts(roleSlot(role, SIZE), job, { agentType, phase: phaseName })
  // agent() throws as well as returns null: a lane that threw would reach parallel() as null and
  // its gap line would read `undefined`, so the exception becomes the blocked line of this stage
  let r = null
  try {
...
let synthesis = null
let bundles = []
// Every exit from here on is built by probeResult() of the shared block: a stage that blocked and
// a run that reached its answer return the same shape, and `ok` is true only when a bundle, the
// critique and the synthesis file all exist.
const result = extra => probeResult({
  out: RESULT, directions: DIRS, cut: CUT, room: SEATS.room, bundles, critique, synthesis, gap,
  blockedStages, ...(extra || {}),
})

// ---- 1. the directions, in parallel: one bundle each, every fact with its pointer ----
phase('Research')
// the lambda hands back the stage object itself: the direction and its number are bound to it
// after the stage, so this flow keeps one exit shape and every `return` below is the result builder
...
for (const r of runs.filter(x => !x.ok)) blockedStages.push(`direction ${r.n} (${r.direction}): ${r.blocked}`)
// the directions this answer is missing: the ones the ceiling never seated and the ones that ran
// and brought nothing — the synthesiser is told about both, or it writes over a hole it cannot see
const MISSING = [...CUT, ...runs.filter(x => !x.ok).map(r => r.direction)]
if (!bundles.length) return result({ stage: 'research', blocked: `every direction came back blocked: ${blockedStages.join(' ;; ')}` })

// ---- 2. the critique: what the bundles do not carry, before an answer is written on them ----
phase('Critique')
...
// The answer stands in one file, every claim with the pointer it rests on; the directions the
// ceiling cut and the directions that came back blocked stand in the return as gaps.
return result({})
