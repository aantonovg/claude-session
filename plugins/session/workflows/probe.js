...
Out: one bundle per direction and one critique beside out, the answer in out, every claim with its pointer.
...
// the gap of the answer, never a second round
const SEATS = stageSeats(DEPTH, DIRS.length)
...
// ---- the flow ----
const gap = CUT.map((d, i) => `direction ${SEATS.seats + i + 1} (${d}) never started: the ceiling of ${SEATS.room} agents per stage ended the research stage`)
const blockedStages = []
let critique = null
let synthesis = null
let bundles = []
...
const result = extra => probeResult({
  out: OUT, directions: DIRS, bundles, critique, synthesis, gap, blockedStages, ...(extra || {}),
})
