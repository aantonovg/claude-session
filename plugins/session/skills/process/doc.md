...
when it is a key document, the evidence chain one slot up in the same class row above its author.
Read `core.md` and `SKILL.md` first.
...
| `result` — the document, written one slot up in the same class row, section by section | `changes`, `runs` | the checklist is answered from the text alone, with no section missing or cut mid-sentence | yes | yes | yes |
...
- A key document — a plan, a decision contract, a specification, a closure report — never reaches
  the user unchecked: the chain runs one slot up in the same class row above its author before the
  user sees it.
