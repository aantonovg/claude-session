...
/* usage:
Runs a scenarios file in real tmux Claude Code sessions and judges transcripts.
scenarios (string, absolute path, required)
runner (string, absolute driver script path, required)
out (string, absolute artifact dir, required)
ids (array of strings, default all)
parser (string, absolute verdict script path, default none)
budget (number, minutes, default 120)
class (string c1-c5, default c3)
submodes (array of strings: no-sonnet no-opus no-fable, default [])
Out: status (completed partial blocked failed), report path (verdicts), summary, failing ids; stage result.md files under <out>/drive and <out>/judge.
Use: behavior test of skill, agent or base rule in fresh sessions.
*/
...
log(`${NAME} | scenarios=${SCN} runner=${RUN} out=${OUT} ids=${IDS.length || 'all'} parser=${PARSER || 'none'} budget=${BUDGET}m cells=${CELL.main}/${CELL.opus}/${CELL.sonnet}`)
...
const stop = (stage, h) => ({ class: CLS, submodes: SUBS, stage, status: h.status, report: h.report, summary: `${stage}: ${h.summary}`, out: OUT, failing: null })

phase('Drive')
const drive = handbackAt(await agent(`Session driver. Scenarios file: ${SCN}. Driver script: ${RUN}. Scenario ids: ${IDS.length ? IDS.join(' ') : 'all (no arguments)'}. Artifact directory: ${OUT} (the driver writes result.log, <id>.jsonl, <id>.pane.txt and done there).
Remove a stale ${OUT}/done first. Start the driver detached with nohup, log to ${OUT}/driver.log, then wait by the poll loop of your definition (each Bash call under 120000 ms) until ${OUT}/done appears or ${BUDGET} minutes pass. Then list the artifacts. Kill every tmux session the driver created before returning.
result.md: artifact directory, result.log path, jsonl count, last driver.log line, sessions killed. Summary: jsonl count and the last driver.log line. No done-file within the budget: status: failed.
${handoff(`${OUT}/drive`)} ${STYLE}`, cellOpts(CELL.sonnet, 'drive', { agentType: 'session-driver', phase: 'Drive' })), `${OUT}/drive`)
if (drive.status !== 'completed') return stop('drive', drive)

phase('Judge')
const judge = handbackAt(await agent(`Transcript analyst. Judge the session run in ${OUT}: scenario definitions and PASS rules in ${SCN}; transcripts ${OUT}/<id>.jsonl; the driver's own verdict lines in ${OUT}/result.log${PARSER ? `; verdict script ${PARSER} (usage: python3 ${PARSER} <jsonl> <id>)` : ''}.
For every scenario decide PASS, FAIL or UNKNOWN over the whole turn (all assistant records after the prompt), quoting the decisive line.
result.md: one line per scenario "<id> PASS|FAIL|UNKNOWN | <decisive quote>", then the counts. Summary: exactly "FAIL: <ids separated by commas, or none>".
${handoff(`${OUT}/judge`)} ${STYLE}`, cellOpts(CELL.sonnet, 'judge', { agentType: 'transcript-analyst', phase: 'Judge' })), `${OUT}/judge`)
if (judge.status !== 'completed') return stop('judge', judge)

const m = judge.summary.match(/^\s*FAIL:\s*(.*)$/i)
const failing = !m ? null : /^none\b/i.test(m[1].trim()) ? [] : [...new Set(m[1].split(/[\s,]+/).filter(Boolean))]
return {
  class: CLS, submodes: SUBS, status: failing ? 'completed' : 'failed', report: judge.report,
  summary: failing ? judge.summary : `judge summary is no FAIL line: ${judge.summary}`, out: OUT, failing,
  next: !failing ? `Read ${judge.report}.` : failing.length ? `Read ${judge.report}; fix the rule behind ${failing.join(', ')}; rerun with ids.` : 'All scenarios pass.',
}
