...
const REV = A.reviews || TRIM.replace(/\/[^/]+$/, '')
const NAME = [CLS, ...SUBS, 'memory-gc'].join('-')
log(`${NAME} | trim=${TRIM} reviews=${REV} slots=${ROW.join('/')}`)

const trimPrompt = (i, review) => `Memory gardener${i > 1 ? `, fix round ${i}` : ''}. Trim list: ${TRIM}${review ? `; review to apply: ${review}` : ''}. Read the list${review ? ', the review' : ''} and the skill it names in one pass.
...
let fixed = null
if (!clean(check)) {
  const f = await agent(trimPrompt(2, `${REV}/gc-review.md`), opts('opus', 'gc-fix', { agentType: 'session:stage-author', phase: 'Check' }))
  fixed = blocked(f) ? `BLOCKED ${last(f)}` : last(f)
}

return {
  class: CLS, submodes: SUBS, slots: SLOT, trim: TRIM, review: `${REV}/gc-review.md`,
  result: clean(check) ? 'clean' : `findings: ${last(check)}; fix: ${fixed}`,
  next: 'Read gc-review.md; restore from git or backups if a fact was lost.',
}
