...
out (string, absolute result directory, default <trim dir>/gc)
class (string c1-c5, default c3)
submodes (array of strings: no-sonnet no-opus no-fable, default [])
Out: status (completed partial blocked failed), report path, summary; stage result.md files under out.
...
const TRIM = A.trim
if (!TRIM || !String(TRIM).startsWith('/')) throw new Error('args.trim (absolute path to memory-trim.md) is required')
const TRIM_DIR = TRIM.slice(0, TRIM.lastIndexOf('/'))
const OUT = normPath(A.out || `${TRIM_DIR}/gc`)
if (!String(OUT).startsWith('/')) throw new Error('args.out must be an absolute directory')
// one backup directory per trim list: its slug is the name of the directory that holds the list
const BACKUP = `${HOME}/.claude/backups/${TRIM_DIR.split('/').pop()}-memory-trim`
const NAME = [CLS, ...SUBS, 'memory-gc'].join('-')
log(`${NAME} | trim=${TRIM} out=${OUT} backup=${BACKUP} cells=${CELL.main}/${CELL.opus}/${CELL.sonnet}`)

const STYLE = 'Plain English, caveman ultra; the return value is data. No skills needed for this step.'
// handoff(dir): the result protocol of every stage; the workflow reads the three lines, never the file
...
// verdict(h): a reviewer's summary is "clean", "clean (<m> medium accepted)" or "findings <n> high <m> medium"
const isClean = h => h.status === 'completed' && /^\s*(VERDICT:\s*)?clean\b/i.test(h.summary)
const stop = (stage, h, extra) => ({ class: CLS, submodes: SUBS, stage, status: h.status, report: h.report, summary: `${stage}: ${h.summary}`, ...(extra || {}) })

const trimPrompt = (dir, review) => `Memory gardener${review ? ', fix round' : ''}. Trim list: ${TRIM}. Read the list and the skill it names in one pass.${review ? ` Apply the open findings of the review ${review}; touch only what they name.` : ''}
For each line: DELETE moves the file with mv into ${BACKUP}/ (mkdir -p first; never rm; a name that already exists there gets a numeric suffix); SHORTEN rewrites the file to its frontmatter plus the kept lines plus one line "Absorbed by skill <name>: <skill path>"; KEEP changes nothing. In the MEMORY.md index next to each touched file (same directory) delete the line of a deleted file and rewrite the line of a shortened file to name the skill. Touch nothing else.
result.md: counts deleted, shortened, kept, index lines changed; one line per touched file with its action; the exceptions. Summary: the four counts.
${handoff(dir)} ${STYLE}`

phase('Trim')
const t = handbackAt(await agent(trimPrompt(`${OUT}/trim`), cellOpts(CELL.opus, 'trim', { agentType: 'session:applier', phase: 'Trim' })), `${OUT}/trim`)
if (t.status !== 'completed') return stop('trim', t, { out: OUT, backup: BACKUP })

phase('Check')
const c = handbackAt(await agent(`Memory reviewer. Trim list: ${TRIM}. Trim result: ${t.report}. Read the list, the skill it names, every SHORTEN file and every MEMORY.md index touched, in one pass. Never edit them.
Findings: a deleted or shortened fact that is absent from the skill (high; the file is in ${BACKUP}/); an index line still naming a deleted file, or missing for a shortened one (medium); a KEEP file that was changed (medium).
result.md: the findings with concrete fixes, or "no findings". Summary: exactly "clean" or "findings <n> high <m> medium".
${handoff(`${OUT}/check`)} ${STYLE}`, cellOpts(CELL.main, 'gc-review', { agentType: 'skill-reviewer', phase: 'Check' })), `${OUT}/check`)
if (c.status !== 'completed') return stop('check', c, { out: OUT, backup: BACKUP, trim: t.report })

if (isClean(c)) return { class: CLS, submodes: SUBS, status: 'completed', report: c.report, summary: `clean; trim: ${t.summary}`, out: OUT, backup: BACKUP, trim: t.report }

const f = handbackAt(await agent(trimPrompt(`${OUT}/fix`, c.report), cellOpts(CELL.opus, 'gc-fix', { agentType: 'session:applier', phase: 'Check' })), `${OUT}/fix`)
return {
  class: CLS, submodes: SUBS, status: f.status === 'completed' ? 'partial' : f.status, report: c.report,
  summary: `${c.summary}; fix: ${f.status} ${f.summary}; the fix is not re-reviewed`,
  out: OUT, backup: BACKUP, trim: t.report, fix: f.report,
  next: `Read the review ${c.report} and the fix ${f.report}; restore from ${BACKUP} or git if a fact was lost.`,
}
